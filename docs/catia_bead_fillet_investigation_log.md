...
### 14.6 フランジ根本BiTangentのUpdate失敗 — 調査と解決(2026-08-25、ユーザー指示)

**症状**: 本番経路のフランジ30件中12件で、根本R(壁x基準面のBiTangent)が全向き
Update失敗。層別では決定的変数なし(凹側跨ぎ75% vs 44%が最有力だが重なる)。

**切り分けの経過**(否定した仮説も記録):
1. 壁は完全(面積が理論値の0.99〜1.01倍) — 部分スイープ説は否定
2. h=8 / 根本R=5 / 拡張ゼロ / 内側後退2mm — いずれも効かず(変種実験)
3. **貫通**: 壁が基準面に「接している」だけだと退化する。下へ3mm貫通で12件中4件回復
   (ビードの壁が延長して作り直されるのと同じ理由、SS6.1)
4. BiTangent引数の総当たり: **support系フラグを0にするとフィレット自体は計算できる**
   (全長分の面積で成立)。失敗しているのはトリム(relimitation)段。ただし成立する
   向きはkeep側が逆の枝のみ
5. **キラリティ反転が決定打**: 残存7件の全てが「方向反転(3/7)か側反転(5/7)」の
   どちらかで成立。成立性は(側x方向x形状)のキラリティに依存する(機構は未解明のまま
   だが、選択肢の中に必ず成立解がある)

**実装**:
- 壁スイープに`SetLength(2, FLANGE_WALL_PIERCE_MM=3.0)`(貫通)
- `flange.chirality_candidates`: 元->方向反転->側反転->両反転の順の候補列
  (方向反転時は凹側交差の高さ制約を再検査し、必要なら高さを詰める)
- `batch_generate.build_general_part`: フランジ起因のValueErrorなら次の候補で再試行。
  メタデータには**実際に使ったflange**を記録する

**結果**: 30/30成立(旧失敗2件・残存6件を含む。うち8件が反転で救済)。

### 14.7 帯狙いサンプリング(ユーザー承認②)

`sample(gentle_folds=True)`: 法線相対角<=35度 + **p2の配置方向を座面平行から±15度に制限**
+ slack目標を「両折れ18度以下」へ切り替え。決定的だったのはp2の配置方向 — 折れ角は
法線の相対角ではなく**p2方向の法線成分**が支配する(法線角だけ絞っても中央値74度のまま
だった)。効果: フランジ到達率 3.6% -> **44%**(帯狙い時)。バッチは
`flange_aim_share`(既定0.4)で補強部品の一部を帯狙いで引く。

### 14.8 batch02のフランジ欠陥3件 — 方向・両側残り・途切れ(2026-08-25、ユーザー指摘)

ユーザーがbatch02の完成品(0003等)で3つの欠陥を発見:
①フランジが**表側**(締結面側)へ伸びる ②基準面を挟んで**両側**にフランジが見える
③フランジが**途中で途切れる**(端パネルが刃状に斜め削れ)。

**調査で否定した仮説**(0003で全て実測):
- 根本曲線・壁のずれ → 全点0.00〜0.04mmで完璧
- 中心線の端クリアランス(1mm手前で止まる) → 端まで届かせても不変
- 端接線の暴れ → オーバーシュート投影でも不変

**確定した機構と対策**:
- ①: 方向をランダムに振っていた設計ミス。**裏側固定**(`backside_direction` —
  締結法線との内積で符号を実測し反対側へ)。相手部品・ボルト座面との干渉回避は
  設計要件であり多様性の対象ではない。
- ②: フィレット成立のための壁貫通3mm(SS14.6)の残骸。正しいトリムなら消える。
  裏側2mmの**removeプローブ**を追加し、残す不正トリムを棄却する。
- ③: 表側ビルドの一部で、フィレットのトリムレールが端パネルを斜めに横断して
  刃状の残骸を作る。パネル中央1点のkeepプローブは素通りしていた —
  **パネルごとに走行2箇所x幅3箇所のkeep**に強化し、斜め削れを棄却する。
  なお0003の形状は裏側が両折れとも凹で、裏側フランジはh=7でも根本フィレットが
  落ちる(=裏側固定の下では正しくビードへフォールバックする形状だった)。
- キラリティ再試行(SS14.6)は方向固定に伴い**側の反転のみ**に縮小。

**検証**: 帯狙い15スペック中、10件がクリーンに成立(レンダリング確認)、5件は
プローブが不正形状を棄却(バッチではビード/次specへ)。中心線の端オーバーシュート
(境界クリップを利用した端接線の安定化)は対策としては不発だったが無害なので残す。

**注意**: batch02のフランジ24部品はこの欠陥の影響下にある。再生成が必要。

---

## 15. 余肉カット — 基準面の隅を締結R で丸める(2026-08-25、ユーザー指定)

補強生成後、基準面の隅を R = 締結点の必要最小半径 で丸めて落とす(実務の重量低減)。
ビード部品は四隅、フランジ部品は**反フランジ側の2隅のみ**(フランジ側は構造が連続)。
R=bearing半径のとき側辺の接点はちょうど締結点の走行位置に来て、丸タブ端形状になる。

実装: `corner_relief.py`(純Python計画: 端パネルの平坦区間に四分円を解析配置、
平坦区間からはみ出せばValueError)+ `gsd_build._apply_corner_relief`
(スプライン→Splitのprobe-and-select。keep=締結点1の座面中心、remove=元の隅)。

**踏んだバグ**: keepを「中央パネルの中心」に置いたら、ビード部品ではそこは基準面が
除去されビード頂面になっているため全ビード部品が棄却され、バッチがフランジだけに
なった(試行数中央7)。keepは**締結点の座面中心**(ビードの逃げ・フランジ根本R・
余肉カットのいずれからも守られる不変点)に置くこと。

検証: スモーク8部品(ビード7/フランジ1)全成功、目視で丸タブ端とフランジ側無傷を確認。
